...
//fixed reported posts shwoing in home after liking any post 
let reportedPost = []
const addToLiked = (id) => {
  let notReportedPost = []
...
const displayLikedPosts = () => {
  ...
  const likedPost = document.getElementById("liked");
  likedPost.innerHTML = '';
  likedPosts.forEach((post) => {
    const div = createPost(post);
    likedPost.appendChild(div);
  });
};

const displayReportedPosts = () => {
  const reportedPosts = getReportedPosts();
  const showReportedPost = document.getElementById("reported");
  showReportedPost.innerHTML = '';
  reportedPosts.forEach((post) => {
    const div = createPost(post);
    showReportedPost.appendChild(div);
  });
};
